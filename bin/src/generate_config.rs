...
fn build_eslint_config(imports: &[String], gitignore_paths: &[&Path]) -> String {
...
pub fn generate_config(packages: Packages) -> Result<()> {
  ...
  if let Some(tsconfig_path) = find_file("tsconfig.json") {
    ...
    if let Some(config) = parsed {
      ...
      let has_empty_files = config
        .files.files.as_ref()
      let include_excludes_configs = config.include.as_ref().is_some_and(|includes| {
      ...
      let needs_default_project = has_empty_files || include_excludes_configs;
  ...
  let eslint_config = build_eslint_config(&eslint_imports, &gitignore_refs);
